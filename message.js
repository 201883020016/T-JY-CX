document.addEventListener('DOMContentLoaded', () => {
    ...
    // clientId：用于后端按客户端隔离 pending 队列
    const getClientId = () => {
    ...
    // ---- 防重复显示：记录已见过的后端 replyId（ACK失败/刷新重复拉取时会用到）----
    const loadSeenReplyIds = () => {
        try {
            const arr = JSON.parse(localStorage.getItem('seenReplyIds') || '[]');
            return new Set(Array.isArray(arr) ? arr : []);
        } catch {
            return new Set();
        }
    };
    const saveSeenReplyIds = (set) => {
        // 限制长度，避免无限长（保留最近 2000 个）
        const arr = Array.from(set);
        const trimmed = arr.length > 2000 ? arr.slice(arr.length - 2000) : arr;
        localStorage.setItem('seenReplyIds', JSON.stringify(trimmed));
    };
    let seenReplyIds = loadSeenReplyIds();
    ...
    const populateGroupFilter = () => {
        ...
        if (uniqueGroups.includes(currentSelection)) zikaGroupFilter.value = currentSelection;
    ...
    const renderZikaReplies = () => {
        ...
        if (selectedGroup && selectedGroup !== '-- 全部 --') {
            if (selectedGroup === '未分组') filteredList = filteredList.filter(r => !r.category);
            else filteredList = filteredList.filter(r => r.category === selectedGroup);
        }
        if (searchTerm) filteredList = filteredList.filter(r => r.content.toLowerCase().includes(searchTerm));
    ...
    const goToNextPage = () => {
        ...
        if (currentPage < totalPages) { currentPage++; renderAllThreads(); }
    };
    ...
    // ACK：前端确认已展示并保存后，通知后端删除 pending
    const ackReplies = (replyIds) => {
        ...
        .then(res => {
            if (!res.ok) throw new Error('ACK失败');
            return res.json();
        })
        .then(data => {
            console.log('ACK成功:', data);
        })
        .catch(err => {
            // ACK 失败不影响展示：下次还会拉到，靠 seenReplyIds 去重
            console.error('ACK请求失败(下次会重试/重复拉取)：', err);
        });
    };

    const displayBackendReplies = async (replies) => {
        // replies: [{replyId, clientId, threadId, sender, content, type, timestamp, createdAt}, ...]
        ...
        for (const threadId in repliesByThread) {
            const thread = threads.find(t => t.id === threadId);
            if (!thread) {
                console.warn('未找到对应话题，先不ACK，等待下次拉取（可能是本地记录尚未同步/被清空）:', {
                    threadId,
                    replies: repliesByThread[threadId]
                });
                continue;
            }

            repliesByThread[threadId].forEach(reply => {
                // 去重：如果这条 replyId 已经见过（可能因为 ACK 失败重复拉取），不要重复展示，但要 ACK 清掉
                if (reply.replyId && seenReplyIds.has(reply.replyId)) {
                    ackIds.push(reply.replyId);
                    return;
                }

                // 转成前端消息对象（不要覆盖后端 replyId）
                const msg = {
                    id: generateId(),                 // 前端渲染用
                    serverReplyId: reply.replyId,      // 用于 ACK/排查
                    sender: reply.sender,
                ...
                thread.lastActivityTimestamp = msg.timestamp;
        ...
        // 先保存“已见过”集合（防止刚显示完就刷新导致重复显示）
        saveSeenReplyIds(seenReplyIds);

        // 先渲染和落库，再 ACK（符合你要的“前端确认显示后回执”语义）
        currentPage = 1;
        renderAllThreads();
        saveData();
    ...
    const fetchPendingReplies = () => {
        ...
        .then(replies => {
            if (replies && replies.length > 0) {
                console.log('拉取到新回复(待ACK):', replies);
    ...
    const addZikaReply = () => {
        ...
        if (content === '') return;
        if (replyLibrary.zika.some(r => r.content === content)) { alert('该字卡已存在。'); return; }
    ...
    const addEmojiReply = () => {
        ...
        if (content === '') return;
        if (replyLibrary.emoji.some(r => r.content === content)) { alert('该Emoji已存在。'); return; }
    ...
    const saveEdit = () => {
        ...
        if (newContent) {
            if (replyLibrary.zika.some((r, i) => r.content === newContent && i !== editingZikaIndex)) { alert('该字卡已存在。'); return; }
    ...
    editZikaCategory.addEventListener('focus', (e) => {
        ...
        e.target.value = '';
        e.target.addEventListener('blur', () => { if (e.target.value === '') e.target.value = originalValue; }, { once: true });
    });
    ...
    const initialize = () => {
        ...
        // 打开/刷新时先拉一次：未 ACK 的会一直在，直到 ACK 成功
        fetchPendingReplies();
