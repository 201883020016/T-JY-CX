document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveConfig = () => { localStorage.setItem('chatSettings', JSON.stringify(chatSettings)); localStorage.setItem('userProfile', JSON.stringify(profile)); };
    const loadData = () => { 
    ...
    const createMessageElement = (msg, threadId) => { 
        ...
        msgDiv.appendChild(timestampDiv); return msgDiv; 
    };
    ...
    // --- 核心逻辑：发起与后端防丢单投递 ---
    const createThread = () => { const text = messageInput.value.trim(); if (!text) return; const now = new Date().toISOString(); const threadId = Date.now().toString(); const msgId = generateId(); const newMessage = { id: msgId, sender: 'user', content: text, type: 'text', timestamp: now, isMain: true, status: 'sending' }; const newThread = { id: threadId, messages: [newMessage], lastActivityTimestamp: now }; threads.push(newThread); messageInput.value = ''; messageInput.style.height = 'auto'; currentPage = 1; renderAllThreads(); triggerDelayedReply(threadId, msgId, 1); saveData(); };
    ...
    const addReplyToThread = (threadId, sender, type, content) => { const thread = threads.find(t => t.id === threadId); if (!thread) return; const now = new Date().toISOString(); const msgId = generateId(); const newReply = { id: msgId, sender, content, type, timestamp: now, isMain: false }; if (sender === 'user') newReply.status = 'sending'; thread.messages.push(newReply); thread.lastActivityTimestamp = now; if (sender === 'user') { triggerDelayedReply(threadId, msgId, 1); } currentPage = 1; renderAllThreads(); saveData(); };
    
    const updateMessageStatus = (threadId, msgId, status) => {
        const thread = threads.find(t => t.id === threadId); if (!thread) return;
        const msg = thread.messages.find(m => m.id === msgId); if (!msg) return;
        msg.status = status; saveData();
        const statusEl = document.querySelector(`.msg-status[data-msg-id="${msgId}"]`);
        if (statusEl) {
            statusEl.className = `msg-status ${status}`;
            if (status === 'sent') { statusEl.innerHTML = '&#10003;'; statusEl.onclick = null; statusEl.title = ""; } 
            else if (status === 'failed') {
                statusEl.innerHTML = '&#8635;'; statusEl.title = "发送给后端失败，点击重试安排";
                statusEl.onclick = () => { if (statusEl.classList.contains('refreshing')) return; statusEl.classList.add('refreshing'); triggerDelayedReply(threadId, msgId, 1); };
            } else { statusEl.innerHTML = ''; statusEl.onclick = null; }
        }
    };
    ...
    const triggerDelayedReply = (threadId, msgId, attempt = 1) => {
        ...
        fetch('/api/schedule-reply', { 
            ...
            body: JSON.stringify({ 
                threadId: threadId, replyLibrary: replyLibrary,
                minDelayMinutes: chatSettings.minDelay, maxDelayMinutes: chatSettings.maxDelay 
            })
        })
        .then(response => { if (!response.ok) throw new Error('网络异常'); return response.json(); })
        .then(data => { console.log('后端安排成功:', data.message); updateMessageStatus(threadId, msgId, 'sent'); })
        .catch(error => { 
    ...
    const displayBackendReplies = (replies) => {
        const repliesByThread = replies.reduce((acc, reply) => { if (!acc[reply.threadId]) { acc[reply.threadId] = []; } acc[reply.threadId].push(reply); return acc; }, {});
        for (const threadId in repliesByThread) {
            const thread = threads.find(t => t.id === threadId);
            if (thread) { repliesByThread[threadId].forEach(reply => { reply.id = generateId(); thread.messages.push(reply); thread.lastActivityTimestamp = reply.timestamp; }); }
        }
        currentPage = 1; renderAllThreads(); saveData();
    };

    const fetchPendingReplies = () => {
        fetch('/api/get-pending-replies')
        .then(response => { if (!response.ok) throw new Error('后端服务异常'); if (response.status === 204) return null; return response.json(); })
        .then(replies => { if (replies && replies.length > 0) { console.log('拉取到新回复:', replies); displayBackendReplies(replies); } })
        .catch(error => { console.error('抓取待领任务失败，稍后自动重试:', error); });
    };

    // 切回前台时，主动拉取一次以防息屏错过
    document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') fetchPendingReplies(); });
    ...
    exportReplyBtn.addEventListener('click', () => { const blob = new Blob([JSON.stringify({ replyLibrary }, null, 2)], { type: 'application/json' }); downloadBlob(blob, `传讯_回复库备份_${new Date().toJSON().slice(0,10)}.json`); backupModal.style.display = 'none';});
    exportChatBtn.addEventListener('click', () => { const blob = new Blob([JSON.stringify({ threads }, null, 2)], { type: 'application/json' }); downloadBlob(blob, `传讯_聊天记录备份_${new Date().toJSON().slice(0,10)}.json`); backupModal.style.display = 'none';});
        ...
        reader.onload = (event) => {
            try {
                ...
                if (importTargetType === 'reply') {
                    if (confirm('警告：将覆盖现有回复库，是否继续？')) { replyLibrary = importedData.replyLibrary || { zika: [], emoji: [], image: [] }; saveData(); location.reload(); }
                } else if (importTargetType === 'chat') {
                    if (confirm('警告：将覆盖所有当前话题和聊天记录，是否继续？')) { threads = importedData.threads || []; saveData(); location.reload(); }
                }
            } catch (err) { alert('导入失败！文件格式异常。'); console.error(err); }
            fileImporter.value = ''; 
        };
    ...
    // 【只保留三个按钮的打开逻辑】
    replyManagerMenuBtn.addEventListener('click', () => { featureDrawer.classList.remove('open'); addZikaFormContainer.classList.remove('open'); showAddZikaFormBtn.classList.remove('open'); replyPanel.classList.add('open'); });
    chatSettingsMenuBtn.addEventListener('click', () => { featureDrawer.classList.remove('open'); minDelayInput.value = chatSettings.minDelay; maxDelayInput.value = chatSettings.maxDelay; fontSizeSlider.value = chatSettings.fontSizeLevel; updateFontSizeDisplay(); settingsModal.style.display = 'flex'; });
    backupMenuBtn.addEventListener('click', () => { featureDrawer.classList.remove('open'); backupModal.style.display = 'flex'; });
    
    // 【修改点：点击返回时，隐藏当前面板，并重新打开上一级的 featureDrawer】
    closePanelBtn.addEventListener('click', () => {
        replyPanel.classList.remove('open');
        featureDrawer.classList.add('open'); // 返回上一级
    });
    ...
    // 设置交互
    const updateFontSizeDisplay = () => { const sizes = { 1: 14, 2: 16, 3: 18, 4: 20, 5: 22 }; fontSizeDisplay.textContent = `${sizes[fontSizeSlider.value]}px`; };
    fontSizeSlider.addEventListener('input', updateFontSizeDisplay);
    
    // 【修改点：让聊天设置的“取消”也具有返回上一级菜单的功能】
    ...
    saveSettingsBtn.addEventListener('click', () => {
        ...
        if (isNaN(minD) || isNaN(maxD) || minD < 2 || maxD > 720 || minD >= maxD) { alert('无效：范围需在2分~12小时内，且最大值需大于最小值。'); return; }
        chatSettings.minDelay = minD; chatSettings.maxDelay = maxD; chatSettings.fontSizeLevel = parseInt(fontSizeSlider.value);
        saveConfig(); applyFontSize();
        settingsModal.style.display = 'none';
        // 保存成功后默认关闭弹窗回到主聊天界面
    });
    ...
    saveProfileBtn.addEventListener('click', () => {
        // 【只改为名字】
        profile.name = editProfileName.value.trim() || '名字'; profile.avatar = editAvatarPreview.src;
        saveConfig(); renderProfile();
    ...
    // 清空与清空确认
    clearDataBtn.addEventListener('click', () => { confirmClearInput.value = ''; confirmClearActionBtn.disabled = true; confirmClearModal.style.display = 'flex'; });
    ...
    const initialize = () => {
        ...
        renderAllThreads();
        renderReplyLibrary();
        tabs.querySelector(`[data-tab="zika"]`).click(); // 确保重置为原版默认打开字卡Tab
        fetchPendingReplies();
